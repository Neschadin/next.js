...
type TraceState =
  | { kind: 'initial' }
  | { kind: 'starting'; name: string; internalName: string }
  | { kind: 'started'; name: string; internalName: string }
  | { kind: 'ending'; name: string; internalName: string }
  | { kind: 'ended' }
...
export class SharedPlaywrightState {
  ...
  async startTrace(name: string): Promise<string> {
    ...
    const internalName = `${this.nextTraceId++}-${encodeURIComponent(name)}-${Date.now()}`
    this.traceState = { kind: 'starting', name, internalName }
    await this.context.tracing.startChunk({ name: internalName })
    this.traceState = { kind: 'started', name, internalName }
  }
  ...
  async endTrace() {
    ...
    const traceOutputPath = path.join(
      ...
      `${path
        ...
        .replace(/\//g, '-')}`,
      `playwright-${this.traceState.internalName}.zip`
    )
    this.traceState = {
      kind: 'ending',
      name: this.traceState.name,
      internalName: this.traceState.internalName,
    }
